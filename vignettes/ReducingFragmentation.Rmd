...
In many spatial planning problems, we are not only interested in meeting conservation targets at minimum cost, but also in whether the solution is overly fragmented. Fragmented solutions can be difficult to manage, may fail to support viable populations, and are often harder to communicate to stakeholders.
...
```{r load-tasmania, echo=FALSE, message=FALSE, warning=FALSE}
# Load data
tas_pu <- get_tas_pu()  
...
Running Cohon calibration on the full dataset (100%) is slow, so it would be worth re-running at 100% on a more powerful machine. Here, we visualise the planning region.

```{r plot-tas-data, fig.width=14, fig.height=11, out.width="100%"}
# Plot
p_tas <- suppressMessages({
  spatialplanr::splnr_plot(
    df        = tas,
    plotTitle = paste0("Tasmania dataset (western ", frac * 100, "% of the study area)")
  ) +
...
```{r solve-baseline, include=FALSE}
...
```{r base-solution, echo=FALSE, fig.width=14, fig.height=11, message=FALSE, warning=FALSE, out.width="100%"}
# Plot
ggSoln <- spatialplanr::splnr_plot(
  df           = base_solution,
  colNames     = "solution_1",
...
In the next sections, we will use this baseline solution as a reference point and compare it with solutions produced by approaches that address fragmentation.

## Helper: fragmentation metrics

We will use a small helper function to compute summary metrics for each solution. This lets us build a single table of results across all methods.
...
summarise_solution <- function(label,
                              ...
                              solution_sf,
                              solve_time_sec,
                              min_patch_size = NULL,   # e.g. from MinPatch (m²)
...
```{r manual-bp-solve, message=FALSE, include=FALSE}
...
```{r manual-bp-plot-low, fig.height=11, fig.width=14, message=FALSE, warning=FALSE, out.width="100%"}
# Plot
ggSoln_bp_low <- spatialplanr::splnr_plot(
  df           = bp_low_solution,
  colNames     = "solution_1",
  plotTitle    = "Manual boundary penalty - Low",
  legendTitle  = "Selected",
  legendLabels = c("No", "Yes")
) +
  spatialplanr::splnr_gg_add(
...
```{r manual-bp-plot-high, fig.height=11, fig.width=14, message=FALSE, warning=FALSE, out.width="100%"}
# Plot
ggSoln_bp_high <- spatialplanr::splnr_plot(
  df           = bp_high_solution,
  colNames     = "solution_1",
  plotTitle    = "Manual boundary penalty - High",
  legendTitle  = "Selected",
  legendLabels = c("No", "Yes")
) +
  spatialplanr::splnr_gg_add(
...
## Approach 2: Automatic calibration with Cohon’s method

### Overview

Choosing boundary penalties by hand can be ad hoc. The function `calibrate_cohon_penalty()` in the *prioritizr* package implements a Cohon-style trade-off calibration. It: 1. Solves the problem for a very cheap but fragmented solution 2. Solves the problem for a very compact but expensive solution; and 3. Uses these two anchor solutions to estimate a boundary penalty that balances cost and fragmentation.

### Calibrate and solve

```{r cohon-calibration, message=FALSE, include=FALSE}
...
```{r plot cohon-calibrated, fig.height=11, fig.width=14, message=FALSE, warning=FALSE, out.width="100%"}
#plot_prioritizr(bp_cohon_solution)

# Plot
ggSoln_cohon_solution <- spatialplanr::splnr_plot(
  df           = bp_cohon_solution,
  colNames     = "solution_1",
  plotTitle    = "Cohon-calibrated",
  legendTitle  = "Selected",
  legendLabels = c("No", "Yes")
) +
  spatialplanr::splnr_gg_add(
...
From the table, we can see that the boundary length is much shorter, however the runtime became considerably longer in comparison to the manual approaches.

## Approach 3: MinPatch – minimum patch size only
...
```{r minpatch-baseline, message=FALSE, include=FALSE}
...
### Visualise and summarise MinPatch

```{r plot Min-patch, fig.height=11, fig.width=14, message=FALSE, warning=FALSE, out.width="100%"}
plot_minpatch(mp3_result, title = "MinPatch Results")
```

```{r plot-Min-patch2, fig.height=11, fig.width=14, message=FALSE, warning=FALSE, out.width="100%"}
mp3_solution <- mp3_result$solution
mp3_solution <- mp3_solution  %>%
  dplyr::rename(solution_1 = minpatch)

# Plot
ggSoln_minpatch <- spatialplanr::splnr_plot(
  df           = mp3_solution,
  colNames     = "solution_1",
  plotTitle    = "MinPatch",
  legendTitle  = "Selected",
...
fragmentation_metrics2 <- dplyr::bind_rows(
  fragmentation_metrics, # change to fragmentation_metrics once Cohon works
  mp3_metrics
)

fragmentation_metrics2
```

We can see from the table that the runtime and the total cost is higher than the manual boundary approach, but far lesser than the Cohon-calibrated approach. Moreover, the number of contiguous patches is less, making the solution more compact.

## Approach 4: MinPatch – minimum patch size with boundary penalty
...
```{r minpatch-baseline-4, message=FALSE, warning=FALSE, include=FALSE}
...
```{r minpatch-map-compare, fig.height=11, fig.width=14, message=FALSE, warning=FALSE, out.width="100%"}
mp4_solution <- mp4_result$solution
mp4_solution <- mp4_solution %>%
  dplyr::rename(solution_1 = minpatch)

# Plot
ggSoln_minpatch_bp <- spatialplanr::splnr_plot(
  df           = mp4_solution,
  colNames     = "solution_1",
  plotTitle    = "MinPatch with Boundary penalty",
  legendTitle  = "Selected",
  legendLabels = c("No", "Yes")
) +
  spatialplanr::splnr_gg_add(
...
fragmentation_metrics3 <- dplyr::bind_rows(
  fragmentation_metrics2,
  mp4_metrics
)

fragmentation_metrics3
```

------------------------------------------------------------------------

Despite adding a boundary penalty, we obtain the same results as the approach that just used Minpatch alone. This is because the minpatch package here has not yet been updated with the corrected function. This approach can thus only be compared accordinlgy once the package has been updated and we obtain the correct results.
...
```{r all-maps-patchwork, fig.width = 20, fig.height = 20, out.width="100%"}
p_base   <- ggSoln
p_low    <- ggSoln_bp_low
p_high   <- ggSoln_bp_high
p_cohon  <- ggSoln_cohon_solution
p_mp3    <- ggSoln_minpatch
p_mp4    <- ggSoln_minpatch_bp
...
```{r final-tradeoff-plot, fig.height = 6, fig.width = 8, out.width="100%"}
ggplot(fragmentation_metrics,
  ...
  labs(
    ...
    title = "Cost–fragmentation trade-off across all fragmentation controls"
  )
```
