---
title: "Exploring Cohon Calibration"
author: "Kris Esturas"
date: "2026-03-02"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Exploring Cohon Calibration}
  %\VignetteEncoding{UTF-8}
  %\VignetteEngine{knitr::rmarkdown}
editor_options: 
  markdown: 
    wrap: 72
---

```{r setup, include=FALSE}
dir.create("_build/figures_Cohon", recursive = TRUE, showWarnings = FALSE)
knitr::opts_chunk$set(
  fig.path   = "_build/figures_Cohon/",
  fig.width  = 8,
  fig.height = 4
)
```

# 1. Introduction

Boundary penalties are used in spatial conservation planning to reduce
fragmentation. In *prioritizr*, `add_boundary_penalties()` encourages
solutions to form **compact clusters** of planning units rather than
**scattered** selections. Compact solutions are often easier to manage
and may better support ecological processes, but they usually come at a
higher cost.

A common challenge is that the boundary penalty is often set
**arbitrarily**. If the penalty is too small, solutions remain
fragmented. If it is too large, solutions become overly compact and can
become very expensive. This makes it hard to judge whether the implied
trade-off between cost and compactness is reasonable.

The function `calibrate_cohon_penalty()` in *prioritizr* provides a more
systematic approach. It applies **Cohon-style calibration** to estimate
a boundary penalty that aims to balance cost and compactness, using two
extreme solutions (see Section 2: A brief note on Cohon-style
calibration).

In this vignette, we use the Tasmania example dataset to explore how
this calibration behaves under simple, controlled changes to the
problem. Specifically, we ask:

1.  How does the calibrated boundary penalty change as we *change the
    conservation target*?
2.  Do target–penalty patterns stay similar when feature complexity is
    reduced (and when changes are applied in reverse order)?
3.  How does the calibrated boundary penalty change as we *change the
    number of features*?
4.  Is the feature–penalty relationship consistent across different
    feature compositions (and in reverse order)?
5.  What are the *runtime implications* of Cohon-style calibration?

The goal is not to find a single “correct” penalty. Instead, we aim to
understand how sensitive the calibration is to problem structure, and
what that means for decision-support tools such as **spatialplanr** and
**shinyplanr**.

# 2. A brief note on Cohon-style calibration

Very briefly, `calibrate_cohon_penalty()`:

1.  Solves an “**ignore boundary**” problem (penalty ≈ 0) to find a very
    cheap but fragmented solution.

2.  Solves a “**boundary-only**” problem (very large penalty) to find a
    very compact but otherwise expensive solution.

3.  Uses these two “anchor” solutions (costs and boundary lengths) to
    compute a **boundary penalty** that trades off cost and boundary
    length in a more balanced way.

4.  Returns this penalty so we can re-solve the problem with a **single
    calibrated value.**

The calibrated penalty therefore depends on how fragmented the cheap
solution is and how expensive the compact solution needs to be. We
therefore explore how changing targets and reducing the number of
features affects the calibrated penalty.

# 3. Data and baseline set-up

We use the Tasmania planning unit and feature data from
*prioritizrdata*. These data represent a realistic spatial conservation
planning problem and are used in other *prioritizr* examples. To keep
the vignette reasonably fast, we crop the study area to a smaller
subregion.

```{r load-packages, echo = FALSE}
# Load packages
library(prioritizr)
library(prioritizrdata)
library(spatialplanr)
library(dplyr)
library(sf)
library(ggplot2)
library(ggrepel)
library(tibble)
library(patchwork)
```

## 3.1 Load and prepare the Tasmania data

The code below prepares the planning units and features for analysis.

```{r planning-region, echo=TRUE}
# Get planning units
tas_pu <- get_tas_pu() %>%
  dplyr::mutate(cost = cost)

# Convert features to sf
tas_features <- get_tas_features() %>%
  stars::st_as_stars() %>%
  sf::st_as_sf()

# Interpolate features to planning units
tas <- sf::st_interpolate_aw(
  tas_features,
  tas_pu,
  extensive = FALSE,
  keep_NA   = FALSE,
  na.rm     = FALSE
) %>%
  sf::st_join(tas_pu, join = sf::st_equals)

# List feature columns
all_features <- tas %>%
  sf::st_drop_geometry() %>%
  dplyr::select(-dplyr::all_of(c("id", "cost", "locked_in", "locked_out"))) %>%
  names()

# Binarise features
tas <- tas %>%
  dplyr::mutate(
    dplyr::across(dplyr::all_of(all_features), ~ dplyr::if_else(.x > 0, 1, 0))
  )

# Crop region
bbox <- tas %>%
  sf::st_bbox()

frac <- 0.50

bbox_small <- bbox
bbox_small["xmax"] <- bbox["xmin"] + frac * (bbox["xmax"] - bbox["xmin"])
bbox_small["ymax"] <- bbox["ymin"] + frac * (bbox["ymax"] - bbox["ymin"])

tas <- tas %>%
  sf::st_crop(bbox_small)

# Remove zero-only features
feature_totals <- tas %>%
  sf::st_drop_geometry() %>%
  dplyr::select(dplyr::all_of(all_features)) %>%
  dplyr::summarise(dplyr::across(dplyr::everything(), ~ sum(.x, na.rm = TRUE)))

zero_features <- feature_totals %>%
  dplyr::select(dplyr::where(~ .x == 0)) %>%
  names()

tas <- tas %>%
  dplyr::select(-dplyr::all_of(zero_features))

all_features <- setdiff(all_features, zero_features)
```

We then visualise the planning units.

```{r plot-plannin-area, echo=FALSE, fig.height=11, fig.width=15, out.width="100%"}
# Plot planning units
p_tas <- suppressMessages({
  spatialplanr::splnr_plot(
    df        = tas,
    plotTitle = "Tasmania dataset (western 50% of the study area)"
  ) +
    ggplot2::geom_sf(data = tas, fill = "grey85", colour = NA) +
    spatialplanr::splnr_gg_add(
      Bndry      = tas,
      colorBndry = scales::alpha("black", 0.5),
      ggtheme    = "Default"
    ) +
    ggplot2::labs(x = NULL, y = NULL)
})
p_tas
```

**Note.** As this example uses only a subset of the planning area for
computational efficiency, absolute results may therefore differ for the
full region or the other subset of the planning area (i.e., eastern 50%
of the study area); thus, merit further experimentation.

# 4. Helper functions

To keep the analysis readable, we create helper functions to summarise
solution characteristics and standardise how problems are built,
calibrated, and solved across scenarios.

## 4.1 Fragmentation and cost metrics

Each solution is summarised using a small set of interpretable metrics,
including selected area, patch structure, boundary length, cost, and
solver runtime.

```{r helper-function}
summarise_solution <- function(label,
                               solution_sf,
                               solve_time_sec,
                               min_patch_size   = NULL,
                               boundary_penalty = 0) {

  # Check inputs
  if (!"solution_1" %in% names(solution_sf)) {
    stop("summarise_solution(): column 'solution_1' not found in solution_sf")
  }
  if (!"cost" %in% names(solution_sf)) {
    stop("summarise_solution(): column 'cost' not found in solution_sf")
  }

  # Select PUs
  sel <- solution_sf %>%
    dplyr::filter(.data$solution_1 == 1)

  n_selected <- sel %>%
    nrow()

  # Return empty summary
  if (n_selected == 0) {
    return(
      tibble::tibble(
        method             = label,
        n_selected         = 0L,
        total_area_km2     = 0,
        n_patches          = 0L,
        valid_patches      = NA_integer_,
        median_patch_km2   = NA_real_,
        pu_cost_total      = 0,
        pu_cost_mean       = NA_real_,
        boundary_length_km = 0,
        boundary_cost      = 0,
        total_cost         = 0,
        solver_time_sec    = solve_time_sec
      )
    )
  }

  # Compute area
  pu_areas_m2 <- sel %>%
    sf::st_area() %>%
    as.numeric()

  total_area_km2 <- pu_areas_m2 %>%
    sum() %>%
    `/`(1e6)

  # Build patches
  patches <- sel %>%
    sf::st_union() %>%
    sf::st_cast("POLYGON")

  patch_areas_m2 <- patches %>%
    sf::st_area() %>%
    as.numeric()

  n_patches <- patch_areas_m2 %>%
    length()

  median_patch_km2 <- patch_areas_m2 %>%
    stats::median() %>%
    `/`(1e6)

  # Count valid patches
  valid_patches <- if (!is.null(min_patch_size)) {
    min_patch_size_m2 <- min_patch_size %>%
      as.numeric()
    patch_areas_m2 %>%
      `>=`(min_patch_size_m2) %>%
      sum()
  } else {
    NA_integer_
  }

  # Measure boundary
  boundary_length_km <- patches %>%
    sf::st_boundary() %>%
    sf::st_cast("MULTILINESTRING") %>%
    sf::st_length() %>%
    as.numeric() %>%
    sum() %>%
    `/`(1000)

  # Summarise costs
  pu_cost_total <- sel %>%
    dplyr::pull(.data$cost) %>%
    sum(na.rm = TRUE)

  pu_cost_mean <- sel %>%
    dplyr::pull(.data$cost) %>%
    mean(na.rm = TRUE)

  boundary_cost <- boundary_length_km * boundary_penalty
  total_cost    <- pu_cost_total + boundary_cost

  # Return summary
  tibble::tibble(
    method             = label,
    n_selected         = n_selected,
    total_area_km2     = total_area_km2,
    n_patches          = n_patches,
    valid_patches      = valid_patches,
    median_patch_km2   = median_patch_km2,
    pu_cost_total      = pu_cost_total,
    pu_cost_mean       = pu_cost_mean,
    boundary_length_km = boundary_length_km,
    boundary_cost      = boundary_cost,
    total_cost         = total_cost,
    solver_time_sec    = solve_time_sec
  )
}
```

## 4.2 Building and calibrating problems

Next, we define a small helper to: 1. build a *prioritizr* problem for a
given **target** and a chosen set of **features**, 2. add a temporary
boundary penalty, 3. run `calibrate_cohon_penalty()` to find a
calibrated boundary penalty, 4. solve the problem with the calibrated
penalty, and 5. return a tidy row with the penalty, timings, and
solution metrics.

```{r build-problem}
# Create function for prioritisation
run_cohon_experiment <- function(label, target, feature_names) {

  prob <- prioritizr::problem(
    x           = tas,
    features    = feature_names,
    cost_column = "cost"
  ) %>%
    prioritizr::add_min_set_objective() %>%
    prioritizr::add_relative_targets(target) %>%
    prioritizr::add_binary_decisions() %>%
    prioritizr::add_rsymphony_solver(verbose = TRUE)

  calib_time <- system.time({
    cohon_penalty <- prioritizr::calibrate_cohon_penalty(
      prob %>% prioritizr::add_boundary_penalties(penalty = 1e-4, edge_factor = 1),
      approx  = TRUE,
      verbose = TRUE
    )
  })

  cohon_penalty <- as.numeric(cohon_penalty)
  if (length(cohon_penalty) != 1 || !is.finite(cohon_penalty)) {
    stop("calibrate_cohon_penalty() did not return a single finite numeric penalty.")
  }

  prob_final <- prob %>%
    prioritizr::add_boundary_penalties(penalty = cohon_penalty, edge_factor = 1)

  solve_time <- system.time({
    solution <- solution <- solve(prob_final, force = TRUE)
  })

  metrics <- summarise_solution(
    label            = label,
    solution_sf      = solution,
    solve_time_sec   = solve_time[["elapsed"]],
    min_patch_size   = NULL,
    boundary_penalty = cohon_penalty
  )
  
  # Create table
  dplyr::mutate(
    metrics,
    scenario       = label,
    n_features     = length(feature_names),
    target         = target,
    cohon_penalty  = cohon_penalty,
    calib_time_sec = calib_time[["elapsed"]],
    total_time_sec = calib_time[["elapsed"]] + solve_time[["elapsed"]]
  )
}
```

# 5. How does the Cohon penalty change with target level?

First, we hold the **set of features constant** and only change the
**target**. Here we use all features and compare targets with
incremental increases of 5% from 0 to 50%.

```{r cohen-targets, message=FALSE, warning=FALSE, include=FALSE}
# 5% steps from 5% to 50%
targets <- seq(0.05, 0.50, by = 0.05)

cohon_target_results <- purrr::map_dfr(
  targets,
  ~ {
    cat("\n========================================\n")
    cat("SECTION 5 | Running target =", .x * 100, "%\n")
    cat("========================================\n")

    run_cohon_experiment(
      label         = paste0("target_", .x * 100),
      target        = .x,
      feature_names = all_features
    )
  }
)

dir.create("_build/ExploringCohon", recursive = TRUE, showWarnings = FALSE)
saveRDS(cohon_target_results, file = "_build/ExploringCohon/cohon_target_results.rds")
```

The table and figures below focus on the calibrated penalty, runtime,
and overall cost structure. Together, they show how increasing targets
influence both the calibration outcome and computational effort.

### 5.1 Table

We summarise the calibrated penalties and associated metrics across
target levels.

```{r cohon-table-results}
# Show table
cohon_target_results %>%
  dplyr::select(
    scenario, n_features, target,
    cohon_penalty,
    calib_time_sec, solver_time_sec, total_time_sec,
    boundary_length_km, boundary_cost, pu_cost_total, total_cost
  ) %>%
  dplyr::arrange(target)
```

**Observations**:

From the table, we can see that:

-   `total_time_sec` varies across targets, with higher values at
    intermediate targets

-   `total_cost` generally increases steadily as the target increases

-   `boundary_length_km` generally increases with target

### 5.2 Plot 1

The first plot shows how the calibrated penalty varies with targets.

```{r cohon-target-plot1, echo=FALSE, message=FALSE, warning=FALSE, error = TRUE, fig.height=5, fig.width=10, out.width="100%"}
# Plot target x cohon penalty
plot1_target <- ggplot(
  cohon_target_results,
  aes(x = target * 100, y = cohon_penalty)
) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 2.2) +
  labs(
    x = "Target (% of each feature)",
    y = "Calibrated boundary penalty",
    title = "How does the Cohon penalty change with target level?"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    panel.border = element_rect(fill = NA, linewidth = 0.6),
    panel.grid.major = element_line(linewidth = 0.25),
    panel.grid.minor = element_blank(),
    plot.title = element_text(face = "bold", size = 16),
    axis.title = element_text(size = 14),
    axis.text  = element_text(size = 12),
    plot.title.position = "plot",
    plot.margin = margin(10, 12, 10, 12)
  )

plot1_target
```

**Observations:**

Calibrated Cohon penalties *generally increase* as conservation targets
become more demanding, then tend to level off at higher targets with
some fluctuations rather than continuing to rise steadily.

### 5.3 Plot 2

The second plot shows the combined runtime (calibration+solve).

```{r cohon-target-plot2, echo=FALSE, message=FALSE, warning=FALSE, error = TRUE, fig.height=5, fig.width=10, out.width="100%"}
# Plot target x cohon penalty
plot2_target <- ggplot(
  cohon_target_results,
  aes(x = target * 100, y = total_time_sec)
) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 2.2) +
  labs(
    x = "Target (% of each feature)",
    y = "Calibration + solve time (s)",
    title = "How does the runtime change across different targets?"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    panel.border = element_rect(fill = NA, linewidth = 0.6),
    panel.grid.major = element_line(linewidth = 0.25),
    panel.grid.minor = element_blank(),
    plot.title = element_text(face = "bold", size = 16),
    axis.title = element_text(size = 14),
    axis.text  = element_text(size = 12),
    plot.title.position = "plot",
    plot.margin = margin(10, 12, 10, 12)
  )

plot2_target
```

**Observations:**

Calibration runtime *peaks at intermediate targets* and then declines
sharply. It then stays consistently low for demanding targets.

# 6. Are target-penalty relationships consistent when feature complexity is reduced?

The previous section varied conservation targets while holding the
feature set fixed (100% of features). Here, we assess the sensitivity of
Cohon-style calibration to feature set size by repeating the target
experiment across predefined feature proportions (**25%, 50%, 75%, and
100%**), with features randomly sampled within each proportion.

To assess whether the observed patterns are robust to execution order,
we repeat the analysis using both forward *(Section 6.1)* and reverse
target sequences *(Section 6.2)*. In our experiment, we will fix the
same subset of features for the increasing target levels.

## 6.1 Forward order (25% → 100%)

```{r targets-features-forward, message=FALSE, warning=FALSE, include=FALSE}
set.seed(123)

# Feature subset proportions to test
targets <- seq(0.05, 0.50, by = 0.05)
feature_props_targets <- c(0.25, 0.50, 0.75, 1.00)
target_label <- function(t) paste0("target_", round(t * 100))
n_all <- length(all_features)

# Feature subsets per proportion (reused across all targets)
feature_subsets_tbl <- purrr::map_dfr(
  feature_props_targets,
  ~{
    prop_f <- .x
    n_sub  <- max(1, floor(prop_f * n_all))

    # seed depends ONLY on feature proportion
    seed_f <- 200000 + as.integer(round(prop_f * 100))
    set.seed(seed_f)

    tibble::tibble(
      feature_prop = prop_f,
      n_features_requested = n_sub,
      seed = seed_f,
      feature_names = list(sample(all_features, size = n_sub))
    )
  }
)

# Run experiment across targets, reusing the same subsets
cohon_target_results_multi <- purrr::map_dfr(
  targets,
  ~{
    t_i <- .x

    cat("\n========================================\n")
    cat("COHON CALIBRATION | TARGET =", t_i * 100, "%\n")
    cat("========================================\n")

    purrr::map_dfr(
      seq_len(nrow(feature_subsets_tbl)),
      ~{
        row_i <- feature_subsets_tbl[.x, ]
        prop_f <- row_i$feature_prop
        n_sub  <- row_i$n_features_requested
        seed_f <- row_i$seed
        feature_subset <- row_i$feature_names[[1]]

        cat("\n----------------------------------------\n")
        cat(
          sprintf(
            "Features: %.0f%% (%d of %d) | Seed: %d\n",
            prop_f * 100, n_sub, n_all, seed_f
          )
        )
        cat("----------------------------------------\n")

        run_cohon_experiment(
          label         = paste0(target_label(t_i), "_feat_", n_sub, "_of_", n_all),
          target        = t_i,
          feature_names = feature_subset
        ) %>%
          dplyr::mutate(
            feature_prop = prop_f,
            n_features_requested = n_sub,
            seed = seed_f
          )
      }
    )
  }
)

saveRDS(cohon_target_results_multi, file = "_build/ExploringCohon/cohon_target_results_multi.rds")
```

### 6.1.1 Table

We summarise the calibrated penalties and associated metrics across
targets and feature subset sizes.

```{r cohon_target_results_multi}
cohon_target_results_multi %>%
  dplyr::select(
    scenario, n_features, target,
    feature_prop, n_features_requested, seed,
    cohon_penalty,
    calib_time_sec, solver_time_sec, total_time_sec,
    boundary_length_km, boundary_cost, pu_cost_total, total_cost
  )
```

### 6.1.2 Plot 1

The first plot shows how the calibrated penalty varies with targets for
each feature proporiton.

```{r cohon-multi-target-plot1, echo=FALSE, message=FALSE, warning=FALSE, error = TRUE, fig.height=5, fig.width=10, out.width="100%"}
# Plot 1
plot1_multi_targets <- ggplot(
  cohon_target_results_multi,
  aes(
    x      = target * 100,
    y      = cohon_penalty,
    colour = factor(feature_prop),
    group  = factor(feature_prop)
  )
) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 2.2) +
  labs(
    x = "Target (% of each feature)",
    y = "Calibrated boundary penalty",
    colour = "Feature proportion",
    title = "How does the Cohon penalty change with target level across different feature subsets?"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    panel.border = element_rect(fill = NA, linewidth = 0.6),
    panel.grid.major = element_line(linewidth = 0.25),
    panel.grid.minor = element_blank(),
    plot.title = element_text(face = "bold", size = 16),
    axis.title = element_text(size = 14),
    axis.text  = element_text(size = 12),
    plot.title.position = "plot",
    plot.margin = margin(10, 12, 10, 12)
  )

plot1_multi_targets
```

**Observations.**

Calibrated boundary penalties *generally increase* with conservation
target across all feature proportions, although penalty magnitudes
differ among feature set sizes, with larger feature sets typically
associated with higher values. Despite these differences, the overall
shape of the penalty–target relationship is similar across feature
proportions, with some variability higher at intermediate targets.

### 6.1.3 Plot 2

The second plot shows the combined runtime (calibration+solve).

```{r cohon-multi-target-plot2, echo=FALSE, message=FALSE, warning=FALSE, error = TRUE, fig.height=5, fig.width=10, out.width="100%"}
# Plot 2
plot2_targets <- ggplot(
  cohon_target_results_multi,
  aes(
    x      = target * 100,
    y      = total_time_sec,
    colour = factor(feature_prop),
    group  = factor(feature_prop)
  )
) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 2.2) +
  labs(
    x = "Target (% of each feature)",
    y = "Calibration + solve time (s)",
    colour = "Feature proportion",
    title = "How does runtime change with target level across different feature subsets?"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    panel.border = element_rect(fill = NA, linewidth = 0.6),
    panel.grid.major = element_line(linewidth = 0.25),
    panel.grid.minor = element_blank(),
    plot.title = element_text(face = "bold", size = 16),
    axis.title = element_text(size = 14),
    axis.text  = element_text(size = 12),
    plot.title.position = "plot",
    plot.margin = margin(10, 12, 10, 12)
  )

plot2_targets
```

**Observations:**

Runtime *generally increases* with the number of features included, with
larger feature sets (75–100%) taking longer to solve than smaller
subsets. Across all feature proportions, runtimes tend to be higher at
lower to intermediate targets and decrease at higher target levels.

**Notes:** Because the same feature subset is held fixed across all
target levels, we cannot assess whether similar patterns would emerge
under alternative feature subsets with different spatial configurations
using a different seed.

## 6.2 Reverse order (100% → 25%)

We are now interested whether the same patterns emerge if we run the
code in reverse order.

```{r targets-features-rev, message=FALSE, warning=FALSE, include=FALSE}
targets <- seq(0.05, 0.50, by = 0.05)
targets_rev <- rev(targets)
feature_props_targets <- c(0.25, 0.50, 0.75, 1.00)
feature_props_targets_rev <- rev(feature_props_targets)
target_label <- function(t) paste0("target_", round(t * 100))
n_all <- length(all_features)

# feature subsets per proportion (reused across all targets)
feature_subsets_tbl <- purrr::map_dfr(
  feature_props_targets,
  ~{
    prop_f <- .x
    n_sub  <- max(1, floor(prop_f * n_all))

    # seed depends ONLY on feature proportion
    seed_f <- 200000 + as.integer(round(prop_f * 100))
    set.seed(seed_f)

    tibble::tibble(
      feature_prop = prop_f,
      n_features_requested = n_sub,
      seed = seed_f,
      feature_names = list(sample(all_features, size = n_sub))
    )
  }
)

cohon_target_results_multi_rev <- purrr::map_dfr(
  targets_rev,
  ~{
    t_i <- .x

    cat("\n========================================\n")
    cat("COHON CALIBRATION [REVERSE] | TARGET =", t_i * 100, "%\n")
    cat("========================================\n")

    purrr::map_dfr(
      feature_props_targets_rev,
      ~{
        prop_f <- .x

        # pull the *same* subset used in the forward run for this prop
        row_i <- dplyr::filter(feature_subsets_tbl, feature_prop == prop_f)
        stopifnot(nrow(row_i) == 1)

        n_sub <- row_i$n_features_requested[[1]]
        seed_f <- row_i$seed[[1]]
        feature_subset <- row_i$feature_names[[1]]

        cat("\n----------------------------------------\n")
        cat(sprintf(
          "[REVERSE] Features: %.0f%% (%d of %d) | Seed: %d\n",
          prop_f * 100, n_sub, n_all, seed_f
        ))
        cat("----------------------------------------\n")

        run_cohon_experiment(
          label         = paste0(target_label(t_i),
                                 "_feat_", n_sub,
                                 "_of_", n_all,
                                 "_rev"),
          target        = t_i,
          feature_names = feature_subset
        ) %>%
          dplyr::mutate(
            feature_prop = prop_f,
            n_features_requested = n_sub,
            seed = seed_f,
            order = "reverse"
          )
      }
    )
  }
)
```

### 6.2.1 Table

We summarise the calibrated penalties and associated metrics across
targets and feature subset sizes.

```{r table-features-rev}
cohon_target_results_multi_rev %>%
  dplyr::select(
    order,
    scenario, n_features, target,
    feature_prop, n_features_requested, seed,
    cohon_penalty,
    calib_time_sec, solver_time_sec, total_time_sec,
    boundary_length_km, boundary_cost, pu_cost_total, total_cost
  ) %>%
  dplyr::arrange(target, n_features_requested) %>%
  knitr::kable(
    digits  = 2,
    caption = "Cohon-calibrated penalties across target levels (reverse order only)."
  )
```

### 6.2.2 Plot 1

Here, we plot the reverse plot together with the forward plot side by
side to compare if similar trends emerge for calibrated penalties with
increasing targets for different feature proportions.

```{r cohon-multi-target-plot1-reverse, echo=FALSE, message=FALSE, warning=FALSE, error = TRUE, fig.height=9, fig.width=10, out.width="100%"}
main_title <- "How does the Cohon penalty change with target level across different feature subsets?"

# Forward plot
p_forward <- plot1_targets + labs(title = "Forward")

# Plot reverse
plot1_targets_reverse <- ggplot(
  cohon_target_results_multi_rev,
  aes(
    x      = target * 100,
    y      = cohon_penalty,
    colour = factor(feature_prop),
    group  = factor(feature_prop)
  )
) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 2.2) +
  labs(
    x = "Target (% of each feature)",
    y = "Calibrated boundary penalty",
    colour = "Feature proportion",
    title = "Reverse"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    panel.border = element_rect(fill = NA, linewidth = 0.6),
    panel.grid.major = element_line(linewidth = 0.25),
    panel.grid.minor = element_blank(),
    plot.title = element_text(face = "bold", size = 16),
    axis.title = element_text(size = 14),
    axis.text  = element_text(size = 12),
    plot.title.position = "plot",
    plot.margin = margin(10, 12, 10, 12)
  )

# Stack plots
(p_forward / plot1_targets_reverse) +
  plot_annotation(
    title = main_title,
    theme = theme(plot.title = element_text(face = "bold", size = 16))
  ) &
  theme(legend.position = "right")
```

**Observations**

-   The resulting plots are identical.

### 6.2.3 Plot 2

Here, we plot the reverse and forwards plots showing the runtimes.

```{r cohon-multi-target-plot2-reverse, echo=FALSE, message=FALSE, warning=FALSE, error=TRUE, fig.height=9, fig.width=10, out.width="100%"}
main_title <- "How does runtime change with target level across different feature subsets?"

# Forward plot 
p_forward <- plot2_targets + labs(title = "Forward")

# Create reverse plot
plot2_targets_reverse <- ggplot(
  cohon_target_results_multi_rev,
  aes(
    x      = target * 100,
    y      = total_time_sec,
    colour = factor(feature_prop),
    group  = factor(feature_prop)
  )
) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 2.2) +
  labs(
    x = "Target (% of each feature)",
    y = "Calibration + solve time (s)",
    colour = "Feature proportion",
    title = "Reverse"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    panel.border = element_rect(fill = NA, linewidth = 0.6),
    panel.grid.major = element_line(linewidth = 0.25),
    panel.grid.minor = element_blank(),
    plot.title = element_text(face = "bold", size = 16),
    axis.title = element_text(size = 14),
    axis.text  = element_text(size = 12),
    plot.title.position = "plot",
    plot.margin = margin(10, 12, 10, 12)
  )

# Stack plots
(p_forward / plot2_targets_reverse) +
  plot_annotation(
    title = main_title,
    theme = theme(plot.title = element_text(face = "bold", size = 16))
  ) &
  theme(legend.position = "right")
```

**Observations:**

-   The runtime plots are not identical probably due to computer load
    when running. However, we can see that generally the same patterns
    emerge. Solvetimes increase from small to intermediate target levels
    and then dramatically decrease and stabilise as the target levels
    become even higher.

# 7. How does the Cohon penalty change with the number of features?

Next, we fix the **target** at 30% and vary the **number of features.**

For each feature proportion (20% to 100% in 10% increments), we run
multiple replicates (20 replicates) using different random feature
subsets. The subsets are chosen at random but with a fixed seed so they
are reproducible.

## 7.1 Forward order (25% → 100%)

```{r cohen-features-forward, message=FALSE, warning=FALSE, include=FALSE}
set.seed(123) 

n_all <- length(all_features)

# 20%, 30%, ..., 100% of features
feature_props <- seq(0.20, 1.00, by = 0.10)

target_fixed <- 0.30
n_reps <- 20  # number of replicates

cohon_feature_results <- purrr::map_dfr(
  feature_props,
  ~{
    prop_i <- .x
    n_sub  <- max(1, floor(prop_i * n_all))

    cat("\n========================================\n")
    cat("COHON CALIBRATION | FEATURE SENSITIVITY\n")
    cat(sprintf(
      "Features: %.0f%% (%d of %d) | Target = %.0f%%\n",
      prop_i * 100, n_sub, n_all, target_fixed * 100
    ))
    cat("========================================\n")

    purrr::map_dfr(
      seq_len(n_reps),
      ~{
        rep_id <- .x

        seed_rep <- 10000 + (n_sub * 100) + rep_id
        set.seed(seed_rep)

        feature_ix     <- sample(n_all, size = n_sub)
        feature_subset <- all_features[feature_ix]

        cat("\n----------------------------------------\n")
        cat(sprintf(
          "Run %d / %d | Seed: %d\n",
          rep_id, n_reps, seed_rep
        ))
        cat("----------------------------------------\n")

        run_cohon_experiment(
          label         = paste0("features_", n_sub, "_of_", n_all, "_run_", rep_id),
          target        = target_fixed,
          feature_names = feature_subset
        ) %>%
          dplyr::mutate(
            feature_prop         = prop_i,
            n_features_requested = n_sub,
            run                  = rep_id,
            seed                 = seed_rep
          )
      }
    )
  }
)
saveRDS(cohon_feature_results, file = "_build/ExploringCohon/cohon_feature_results.rds")
```

### 7.1.1 Table

We summarise the calibrated penalties and associated metrics across
targets and feature subset sizes.

```{r table-features-forward}
cohon_feature_results %>%
  select(
    scenario, n_features, target,
    cohon_penalty,
    calib_time_sec, solver_time_sec, total_time_sec,
    boundary_length_km, boundary_cost, pu_cost_total, total_cost,
    feature_prop, run, seed
  ) %>%
  arrange(n_features, run) %>%
  knitr::kable(
    digits  = 2,
    caption = paste0(
      "Cohon-calibrated penalties for different numbers of features (target 30%), ",
      "with ", n_reps, " random feature-subset runs per feature count."
    )
  )
```

### 7.1.2 Plot 1

The first plot shows how the calibrated boundary penalty changes as the
number of features increase, while looking at different random subsets.

```{r Plot-features-forward, echo=FALSE, message=FALSE, warning=FALSE, error = TRUE, fig.height=6, fig.width=10, out.width="100%"}
# Plot
plot1_features <- ggplot(
  cohon_feature_results,
  aes(
    x      = n_features,
    y      = cohon_penalty,
    colour = factor(run),
    group  = factor(run)
  )
) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 2.2) +
  labs(
    x      = "Number of features in the problem",
    y      = "Calibrated boundary penalty",
    colour = "Run",
    title  = "How does the Cohon penalty change with the number of features? (random subsets)"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    panel.border       = element_rect(fill = NA, linewidth = 0.6),
    panel.grid.major   = element_line(linewidth = 0.25),
    panel.grid.minor   = element_blank(),
    plot.title         = element_text(face = "bold", size = 16),
    axis.title         = element_text(size = 14),
    axis.text          = element_text(size = 12),
    plot.title.position = "plot",
    plot.margin        = margin(10, 12, 10, 12)
  )

plot1_features
```

**Observations**:

-   There is not a strict general pattern for the relationship between
    the calibrated boundary penalty and increasing number of features.
    However, it can be said that there is slight **general increase**
    with the calibrated penalty as the number of features increase.

### 7.1.3 Plot 2

The plot here shows the relationship between runtimes and increasing
number of features.

```{r Plot2-features-forward, echo=FALSE, message=FALSE, warning=FALSE, error = TRUE, fig.height=6, fig.width=10, out.width="100%"}
# Plot
plot2_features <- ggplot(
  cohon_feature_results,
  aes(
    x      = n_features,
    y      = total_time_sec,
    colour = factor(run),
    group  = factor(run)
  )
) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 2.2) +
  labs(
    x      = "Number of features in the problem",
    y      = "Calibration + solve time (s)",
    colour = "Run",
    title  = "Runtime for Cohon calibration at different feature counts (random subsets)"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    panel.border        = element_rect(fill = NA, linewidth = 0.6),
    panel.grid.major    = element_line(linewidth = 0.25),
    panel.grid.minor    = element_blank(),
    plot.title          = element_text(face = "bold", size = 16),
    axis.title          = element_text(size = 14),
    axis.text           = element_text(size = 12),
    plot.title.position = "plot",
    plot.margin         = margin(10, 12, 10, 12)
  )

plot2_features
```

**Observations:**

-   Due to the outlier, which shows a very long runtime, it is difficult
    to look at the overall pattern. Nonetheless, it can be said that the
    calibration time *generally slighlty increases* with an increase in
    the number of features.

**Notes:**

-   The runtime might be affected by the computational capacity of the
    computer as well as the use of other apps while the code was
    running.

-   It is important to note that the feature draw used for a given run
    (for example, Run 1 with 5 of 24 features) is independent of the
    draw used for the same run at a different feature size (for example,
    Run 1 with 7 of 24 features). The random seed differs across feature
    sizes, meaning that feature subsets are not nested and do not build
    on one another. As a result, it may be useful in future work to
    explore a nested design in which feature subsets at larger sizes are
    constructed by adding features to already existing smaller subsets.

## 7.2 Reverse order (100% → 25%)

To check whether evaluation order affects results, we repeat the same
experiment but iterate through feature proportions in reverse order
(100% down to 20%). In principle, results should be consistent given
identical random seeds.

```{r features-reverse, message=FALSE, warning=FALSE, include=FALSE}
set.seed(123) 

n_all <- length(all_features)

# REVERSE: 100%, 90%, ..., 20% of features
feature_props <- seq(1.00, 0.20, by = -0.10)

target_fixed <- 0.30
n_reps <- 20   # number of replicates

cohon_feature_results_rev <- purrr::map_dfr(
  feature_props,
  ~{
    prop_i <- .x
    n_sub  <- max(1, floor(prop_i * n_all))

    cat("\n========================================\n")
    cat("COHON CALIBRATION [REVERSE] | FEATURE SENSITIVITY\n")
    cat(sprintf(
      "Number of features: %d/%d (%.0f%%) | Target = %.0f%%\n",
      n_sub, n_all, prop_i * 100, target_fixed * 100
    ))
    cat("========================================\n")

    purrr::map_dfr(
      rev(seq_len(n_reps)),   # 20, 19, ..., 1
      ~{
        rep_id <- .x

        # same seed logic as forward (if forward used this formula too)
        seed_rep <- 10000 + (n_sub * 100) + rep_id
        set.seed(seed_rep)

        feature_ix     <- sample(n_all, size = n_sub)
        feature_subset <- all_features[feature_ix]

        cat("\n----------------------------------------\n")
        cat(sprintf(
          "Run %d/%d | Seed: %d\n",
          rep_id, n_reps, seed_rep
        ))
        cat("----------------------------------------\n")

        run_cohon_experiment(
          label         = paste0("features_", n_sub, "_of_", n_all, "_run_", rep_id, "_rev"),
          target        = target_fixed,
          feature_names = feature_subset
        ) %>%
          dplyr::mutate(
            feature_prop         = prop_i,
            n_features_requested = n_sub,
            run                  = rep_id,
            seed                 = seed_rep,
            order                = "reverse"
          )
      }
    )
  }
)
saveRDS(cohon_feature_results_rev, file = "_build/ExploringCohon/cohon_feature_results_rev.rds")
```

### 7.2.1 Table

We summarise the calibrated penalties and associated metrics across
targets and feature subset sizes.

```{r table-features-reverse}
cohon_feature_results_rev %>%
  select(
    scenario, n_features, target,
    cohon_penalty,
    calib_time_sec, solver_time_sec, total_time_sec,
    boundary_length_km, pu_cost_total, total_cost,
    feature_prop, run, seed, order
  ) %>%
  arrange(n_features, run) %>%
  knitr::kable(
    digits  = 2,
    caption = paste0(
      "REVERSE order: Cohon-calibrated penalties for different numbers of features (target 30%), ",
      "with ", n_reps, " random feature-subset runs per feature count."
    )
  )
```

### 7.2.2 Plot 1

Here, we plot both reverse and forward orders side by side to show
whether we obtain the same trend for the relationship between calibrated
boundary penalty and the increasing number of features.

```{r Plot-features-forward-reverse, echo=FALSE, message=FALSE, warning=FALSE, error=TRUE, fig.height=12, fig.width=10, out.width="100%"}

main_title <- "How does the Cohon penalty change with the number of features? (random subsets)"

# -----------------------------
# Forward plot
# -----------------------------
p_forward <- ggplot(
  cohon_feature_results,
  aes(
    x      = n_features,
    y      = cohon_penalty,
    colour = factor(run),
    group  = factor(run)
  )
) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 2.2) +
  labs(
    x      = "Number of features in the problem",
    y      = "Calibrated boundary penalty",
    colour = "Run",
    title  = "Forward"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    panel.border        = element_rect(fill = NA, linewidth = 0.6),
    panel.grid.major    = element_line(linewidth = 0.25),
    panel.grid.minor    = element_blank(),
    plot.title          = element_text(face = "bold", size = 16),
    axis.title          = element_text(size = 14),
    axis.text           = element_text(size = 12),
    plot.title.position = "plot",
    plot.margin         = margin(10, 12, 10, 12)
  )

# -----------------------------
# Reverse plot
# (assumes you have cohon_feature_results_rev already prepared)
# -----------------------------
p_reverse <- ggplot(
  cohon_feature_results_rev,
  aes(
    x      = n_features,
    y      = cohon_penalty,
    colour = factor(run),
    group  = factor(run)
  )
) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 2.2) +
  labs(
    x      = "Number of features in the problem",
    y      = "Calibrated boundary penalty",
    colour = "Run",
    title  = "Reverse"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    panel.border        = element_rect(fill = NA, linewidth = 0.6),
    panel.grid.major    = element_line(linewidth = 0.25),
    panel.grid.minor    = element_blank(),
    plot.title          = element_text(face = "bold", size = 16),
    axis.title          = element_text(size = 14),
    axis.text           = element_text(size = 12),
    plot.title.position = "plot",
    plot.margin         = margin(10, 12, 10, 12)
  )

# -----------------------------
# Stack plots + shared title
# -----------------------------
(p_forward / p_reverse) +
  patchwork::plot_annotation(
    title = main_title,
    theme = theme(plot.title = element_text(face = "bold", size = 16))
  ) &
  theme(legend.position = "right")
```

**Observations:**

-   Identical plots are obtained.

### 7.2.3 Plot 2

Here, we plot both reverse and forward orders side by side to show
whether we obtain the same trend for the relationship between the
runtimes and the increasing number of features.

```{r Plot-features-runtime-forward-reverse, echo=FALSE, message=FALSE, warning=FALSE, error=TRUE, fig.height=12, fig.width=10, out.width="100%"}

main_title <- "How does runtime change with the number of features? (random subsets)"

# -----------------------------
# Forward plot
# (uses your existing plot2_features object)
# -----------------------------
p_forward <- plot2_features + labs(title = "Forward")

# -----------------------------
# Reverse plot
# -----------------------------
plot2_features_reverse <- ggplot(
  cohon_feature_results_rev,
  aes(
    x      = n_features,
    y      = total_time_sec,
    colour = factor(run),
    group  = factor(run)
  )
) +
  geom_line(linewidth = 0.8) +
  geom_point(size = 2.2) +
  labs(
    x      = "Number of features in the problem",
    y      = "Calibration + solve time (s)",
    colour = "Run",
    title  = "Reverse"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    panel.border        = element_rect(fill = NA, linewidth = 0.6),
    panel.grid.major    = element_line(linewidth = 0.25),
    panel.grid.minor    = element_blank(),
    plot.title          = element_text(face = "bold", size = 16),
    axis.title          = element_text(size = 14),
    axis.text           = element_text(size = 12),
    plot.title.position = "plot",
    plot.margin         = margin(10, 12, 10, 12)
  )

# -----------------------------
# Stack plots
# -----------------------------
(p_forward / plot2_features_reverse) +
  patchwork::plot_annotation(
    title = main_title,
    theme = theme(plot.title = element_text(face = "bold", size = 16))
  ) &
  theme(legend.position = "right")
```

**Observations:**

-   The runtime for both forward and reverse orders are generally the
    same, but not completely identical.

# 8. Take home messages

1.  Calibrated boundary penalties can change with conservation targets.

2.  Penalties are sensitive to feature richness and composition because
    calibration responds to spatial structure.

3.  Calibration introduces computational overhead that should be
    considered in applied planning workflows.
