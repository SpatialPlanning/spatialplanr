...
dir.create("figures_Cohon", recursive = TRUE, showWarnings = FALSE)
knitr::opts_chunk$set(
  fig.path = "figues_Cohon/",
  fig.width = 8,
  fig.height = 4
)
```
...
more systematic approach. It applies **Cohon-style calibration** to
estimate a boundary penalty that aims to balance cost and compactness,
using two extreme solutions (see Section 2: A brief note on Cohon-style
calibration).
...
saveRDS(cohon_target_results, file = "vignettes/data/cohon_target_results.rds")
```
...
```{r cohon-target-plot1, echo=FALSE, message=FALSE, warning=FALSE, error = TRUE}

plot1_target <- ggplot(cohon_target_results,
...
```{r cohon-target-plot 2, echo=FALSE}
ggplot(cohon_target_results, 
       aes(x = target * 100, y = total_time_sec)) +
  geom_line() +
  geom_point() +
  theme_minimal() +
  labs(x = "Target (% of each feature)",
...
saveRDS(cohon_target_results_multi, file = "vignettes/data/cohon_target_results_multi.rds")
```
